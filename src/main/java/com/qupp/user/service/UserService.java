...
public class UserService {
    ...
    @Transactional
    public ResponseUser updatePassword(Long id, String password) {
        User user = userRepository.findById(id).orElseThrow(() -> new NoSuchElementException("존재하지 않는 회원입니다."));

        if(!(isEqualPassword(user.getEmail(), password))) {
            throw new BadCredentialsException("기존 비밀번호가 일치하지 않아 변경에 실패했습니다");
        }
    ...
    @Transactional
    public void sendPasswordEmail(ResponseUser responseUser) throws MessagingException {
        String message = "QUPP 임시 비밀번호입니다.";

        User user = userRepository.findByEmail(responseUser.getEmail()).get();

        String tmpPassword = UUID.randomUUID().toString().replace("-", "");
        tmpPassword = tmpPassword.substring(0, 10);

        updatePassword(user.getId(), tmpPassword);
    ...
    public boolean isEqualPassword(String email, String password) {
    ...
    public String genAccessToken(User user) {
        return jwtProvider.generateAccessToken(user.getAccessTokenClaims(), 60 * 60 * 60 * 24 * 365);
    }
}
